...
for item in unpacker:
    ...
    for key in a["ex_data"].keys():
        url = str(key.encode("utf-8"))
        #print "URL: "+ url
        puc_url_list.append(url)
        indiv_query = []
        if not (a["ex_data"].get(key) is None):
            list3 = a["ex_data"].get(key)
            if not list3 is None:
                ...
                if not ucrawl_queries is None:
                    for q in ucrawl_queries:
                        for tr in q:
                            if (isinstance(tr,int)):
                                continue
                            else:
                                #print str("[ucrawl]\t")+tr.encode("utf-8")
                                queries_returned_list.append(tr.encode("utf-8"))
                                indiv_query.append(tr.encode("utf-8"))
