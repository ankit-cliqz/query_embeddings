            ...
            if not list3 is None:
                ...
                if not ucrawl_queries is None:
                    for q_ucrawl in ucrawl_queries:
                        if len(q_ucrawl)== 2:
                            #print(len(q_ucrawl))
                            query_temp0 = q_ucrawl[0].encode("utf-8")
                            query_freq0 = q_ucrawl[1]
                            #print (query_temp0+"\t"+url+"\t"+str(query_freq0)+"\t"+"ucrawl"+"\n")
                            fh.write(query_temp0+"\t"+url+"\t"+str(query_freq0)+"\t"+"ucrawl"+"\n")
                if not qc_queries is None:
                    for q_qc in qc_queries:
                        if len(q_qc) == 2:
                            #print(len(q_qc))
                            query_temp1 = q_qc[0].encode("utf-8")
                            query_freq1 = q_qc[1]
                            #print(query_temp1+"\t"+url+"\t"+str(query_freq1)+"\t"+"census"+"\n")
                            fh.write(query_temp1+"\t"+url+"\t"+str(query_freq1)+"\t"+"census"+"\n")

                if not qs_queries is None:
                    for q_qs in qs_queries:
                        if len(q_qs) == 2:
                            #print(len(q_qs))
                            query_temp2 = q_qs[0].encode("utf-8")
                            query_freq2 = q_qs[1]
                            #print(query_temp2+"\t"+url+"\t"+str(query_freq2)+"\t"+"similar"+"\n")
                            fh.write(query_temp2+"\t"+url+"\t"+str(query_freq2)+"\t"+"similar"+"\n")
